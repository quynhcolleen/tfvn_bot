...
class RollCog(commands.Cog):
    ...
    @commands.command(
        name="roleroll", help="Gives a specified role to a mentioned member."
    )
    @commands.has_permissions(
        manage_roles=True
    )  # Optional: ensures only users with role management permission can use the command
    async def give_role(self, ctx, member: discord.Member, *, role_name: str):
        ...
        try:
            ...
            await ctx.send(
                f"Đã gán vai trò `{role.name}` cho {member.mention} thành công!"
            )

        except discord.Forbidden:
            await ctx.send(
                "Bot không có quyền cần thiết để gán vai trò này. Vui lòng kiểm tra thứ bậc vai trò."
            )
    ...
    @commands.command(
        name="roleunroll", help="Removes a specified role from a mentioned member."
    )
    ...
    async def remove_role(self, ctx, member: discord.Member, *, role_name: str):
        ...
        try:
            ...
            await ctx.send(
                f"Đã gỡ vai trò `{role.name}` khỏi {member.mention} thành công!"
            )

        except discord.Forbidden:
            await ctx.send(
                "Bot không có quyền cần thiết để gỡ vai trò này. Vui lòng kiểm tra thứ bậc vai trò."
            )
        except Exception as e:
            await ctx.send("Đã xảy ra lỗi không mong muốn")
